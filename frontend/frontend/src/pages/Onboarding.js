    ...
    switch (currentStep) {
      case 1:
        return (
          <div className="text-center">
            ...
            <h2 className="text-3xl font-bold mb-4">{steps[0].title}</h2>
            <p className="text-xl text-gray-400 mb-8">{steps[0].subtitle}</p>
      ...
      case 2:
        return (
          <div>
            ...
            <p className="text-xl text-gray-400 mb-8 text-center">{steps[1].subtitle}</p>
            <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
              {businessTypes.map((type) => (
                <button
                  key={type.id}
                  onClick={() => handleBusinessSelect(type.id)}
                  className={`card p-6 text-center transition-all ${formData.businessType === type.id ? 'border-2 border-primary-500 bg-primary-500/10' : 'hover:border-primary-500/30'}`}
                >
      ...
      case 3:
        return (
          <div className="text-center">
            <div className="text-6xl mb-6">📱</div>
            <h2 className="text-3xl font-bold mb-4">{steps[2].title}</h2>
            <p className="text-xl text-gray-400 mb-8">{steps[2].subtitle}</p>
            <div className="max-w-md mx-auto">
              <div className="card p-8 mb-6">
                ...
                <p className="text-gray-400 mb-4">واتس آب هو القناة الوحيدة النشطة حالياً في AutoFlow</p>
                <button
      ...
      case 4:
        return (
          <div>
            ...
            <p className="text-xl text-gray-400 mb-8 text-center">{steps[3].subtitle}</p>
                ...
                return (
                  ...
                  >
                    <div className="flex items-center gap-3 mb-3">
                      <Zap className={`w-5 h-5 ${selected ? 'text-whatsapp' : 'text-gray-500'}`} />
                      <h3 className="font-bold">{reply.name}</h3>
                    </div>
                    <p className="text-sm text-gray-400">{reply.response.substring(0, 50)}...</p>
                    {selected && <div className="mt-3 flex items-center gap-1 text-whatsapp text-sm"><Check className="w-4 h-4" /> مضاف</div>}
                  </button>
                );
              })}
            </div>
            <p className="text-center text-gray-500 text-sm mt-4">تقدر تعدل أو تضيف قواعد تانية من لوحة التحكم بعدين</p>
          </div>
        );
      ...
      case 5:
        return (
          <div className="text-center">
            <div className="text-6xl mb-6">🎉</div>
            <h2 className="text-3xl font-bold mb-4">{steps[4].title}</h2>
            <p className="text-xl text-gray-400 mb-8">{steps[4].subtitle}</p>
            ...
            <button
              ...
              disabled={loading}
              className="btn-gradient px-10 py-4 text-xl font-bold"
            >
      ...
      <div className="w-full max-w-4xl">
          ...
          <div className="flex items-center justify-between mb-4">
            {steps.map((step, index) => (
              <div key={step.id} className="flex items-center">
                <div className={`w-10 h-10 rounded-full flex items-center justify-center text-sm font-bold ${currentStep >= step.id ? 'bg-primary-500 text-white' : 'bg-dark-800 text-gray-500'}`}>
                  {step.id}
                </div>
                {index < steps.length - 1 && <div className={`w-12 md:w-24 h-1 mx-2 ${currentStep > step.id ? 'bg-primary-500' : 'bg-dark-800'}`} />}
              </div>
            ))}
          </div>
        </div>
        ...
        <div className="flex justify-between mt-8">
          <button
            ...
            className={`flex items-center gap-2 px-6 py-3 rounded-lg ${currentStep === 1 ? 'opacity-0' : 'btn-outline'}`}
          >
