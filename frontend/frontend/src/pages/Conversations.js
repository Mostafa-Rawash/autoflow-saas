    ...
    <div className="space-y-6">
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
        <div>
          ...
          <p className="text-gray-400 mt-1">إدارة محادثات واتس آب فقط في مكان واحد</p>
        </div>
        <div className="flex items-center gap-2">
          <span className="status-active px-3 py-1 text-sm">{mockConversations.filter(c => c.status === 'active').length} نشطة</span>
      ...
      <div className="space-y-3">
        {filteredConversations.map((conv) => (
          <Link key={conv.id} to={`/conversations/${conv.id}`} className="card p-4 hover:border-primary-500/30 transition-colors block">
            <div className="flex items-center gap-4">
              <div className="w-12 h-12 rounded-full flex items-center justify-center text-xl font-bold flex-shrink-0 bg-whatsapp/20">📱</div>
              <div className="flex-1 min-w-0">
                ...
                <div className="flex items-center gap-2 mb-1">
                  <span className="text-sm text-whatsapp flex items-center gap-1">📱 whatsapp</span>
              ...
              <div className="flex flex-col items-end gap-2 flex-shrink-0">
                <span className={`inline-block px-3 py-1 rounded-full text-xs ${conv.status === 'active' ? 'status-active' : conv.status === 'pending' ? 'status-pending' : 'status-resolved'}`}>
      ...
      {filteredConversations.length === 0 && (
        <div className="card p-8 text-center">
          ...
          <p className="text-gray-400">جرب تغيير البحث أو الفلتر</p>
        </div>
      )}
    </div>
  );
