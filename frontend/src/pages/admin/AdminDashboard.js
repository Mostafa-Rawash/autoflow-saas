    ...
    <div className="space-y-6">
      ...
      <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
        <div>
          ...
          <p className={`text-sm mt-1 ${theme === 'light' ? 'text-slate-500' : 'text-gray-400'}`}>نظرة عامة على النظام</p>
        </div>
      ...
      <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
        ...
        <div className="glass rounded-xl p-5">
          <div className="flex items-center justify-between">
            <div>
              <p className={`text-sm ${theme === 'light' ? 'text-slate-500' : 'text-gray-400'}`}>المحادثات</p>
        ...
        <div className="glass rounded-xl p-5">
          <div className="flex items-center justify-between">
            <div>
              <p className={`text-sm ${theme === 'light' ? 'text-slate-500' : 'text-gray-400'}`}>مشاهدات المحتوى</p>
            ...
            {chartData.map((item, idx) => (
              <div key={idx} className="space-y-2">
                <div className="flex justify-between text-sm">
                  ...
                  <span className={theme === 'light' ? 'text-slate-500' : 'text-gray-400'}>{item.users} مستخدم • {item.revenue.toLocaleString()} ج.م</span>
                </div>
                <div className="flex gap-2">
                  <div className={`flex-1 h-2 rounded-full overflow-hidden ${theme === 'light' ? 'bg-slate-200' : 'bg-dark-800'}`}>
                    <div
        ...
        <div className="glass rounded-xl p-6">
          ...
          <div className="space-y-4">
            {recentActivity.map((activity, idx) => (
              <div key={idx} className={`flex items-start gap-3 p-3 rounded-lg ${theme === 'light' ? 'bg-slate-50' : 'bg-dark-800/50'}`}>
                ...
                <div className="flex-1">
                  ...
                  <p className={`text-xs ${theme === 'light' ? 'text-slate-500' : 'text-gray-400'}`}>{activity.details}</p>
                </div>
                <span className={`text-xs ${theme === 'light' ? 'text-slate-400' : 'text-gray-500'}`}>{activity.time}</span>
              </div>
            ))}
          </div>
        </div>
        ...
        <div className="grid md:grid-cols-4 gap-4">
          <div className="flex items-center gap-3">
            ...
            <div>
              ...
              <p className={`text-xs ${theme === 'light' ? 'text-slate-500' : 'text-gray-400'}`}>متصل</p>
            </div>
          </div>
          <div className="flex items-center gap-3">
            <div className="w-3 h-3 rounded-full bg-green-400 animate-pulse"></div>
            <div>
              ...
              <p className={`text-xs ${theme === 'light' ? 'text-slate-500' : 'text-gray-400'}`}>نشط</p>
            </div>
          </div>
          <div className="flex items-center gap-3">
            <div className="w-3 h-3 rounded-full bg-green-400 animate-pulse"></div>
          ...
          <div className="flex items-center gap-3">
            ...
            <div>
              ...
              <p className={`text-xs ${theme === 'light' ? 'text-slate-500' : 'text-gray-400'}`}>78% مستخدم</p>
            </div>
          </div>
        </div>
      </div>
