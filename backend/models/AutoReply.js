...
autoReplySchema.statics.findMatching = async function(userId, message, organization = null) {
  const query = {
    isActive: true,
    channel: 'whatsapp',
    $or: [{ user: userId }]
  ...
  if (organization) {
    query.$or.push({ organization });
  }

  const rules = await this.find(query).sort({ priority: -1, usageCount: 1 });
