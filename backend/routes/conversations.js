...
router.post('/:id/messages', auth, async (req, res) => {
  try {
    ...
    try {
      ...
      const externalId = conversation.contact?.externalId;
      const phone = conversation.contact?.phone;
      const forwardTarget = externalId || (phone ? `${String(phone).replace(/[^0-9]/g, '')}@c.us` : null);
      const normalizedTarget = String(forwardTarget || '').replace(/[^0-9@c.us]/g, '');
      console.log('[conversations] forwarding message to WhatsApp', {
      ...
      if (normalizedTarget) {
        await whatsappService.sendMessage(req.user.id, forwardTarget, content, { media, buttons });
        whatsappSent = true;
      } else {
        whatsappError = 'Missing WhatsApp target on conversation';
        console.warn('[conversations] skipped WhatsApp forward due to missing/invalid target', {
          conversationId: conversation._id?.toString?.() || String(conversation._id),
          externalId,
          phone,
          forwardTarget
      ...
      whatsappError = sendError?.message || String(sendError);
      console.error('Error forwarding conversation message to WhatsApp:', whatsappError, sendError?.stack);
    }
    ...
    if (!whatsappSent) {
      ...
      return res.status(201).json({ success: true, message, whatsappSent: false, whatsappError });
    }

    res.status(201).json({ success: true, message, whatsappSent: true });
  } catch (err) {
    console.error(err);
    res.status(500).json({ error: 'Server error' });
  }
